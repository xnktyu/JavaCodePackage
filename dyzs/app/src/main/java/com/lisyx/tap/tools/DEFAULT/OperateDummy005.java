...
public class OperateDummy005 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy005(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy005.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy005.readRunSpeed(context);
		runTarget = ActivityDummy005.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy005.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy005.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy005.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy005.addRunRecord(context);
						startOne();
					}
				}
			}
