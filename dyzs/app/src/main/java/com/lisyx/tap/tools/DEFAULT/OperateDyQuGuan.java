...
public class OperateDyQuGuan extends OperateBase
{
	...
	public OperateDyQuGuan(DyAccessibilityService service)
	{
	...
	public void start()
	{
		allCount = ActivityDyQuGuan.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDyQuGuan.readRunSpeed(context);
	...
	private void clickGuanZhu(final AccessibilityNodeInfo recyclerNode, final int index)
	{
		if (index < recyclerNode.getChildCount())
		{
			service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
			{
				...
				public void process()
				{
					AccessibilityNodeInfo nodeFind = service.clickIfFindOne(recyclerNode.getChild(index), "已关注", new NodeHelper.OnFindCallback()
					{
						@Override
						public boolean find(AccessibilityNodeInfo theNode, int indent, String className, String text, String contentDescription, String viewId)
						{
							return !TextUtils.isEmpty(className) && className.equals("android.widget.TextView") && //
									!TextUtils.isEmpty(text) && text.equals("已关注");
						}
					}, null);
					if (nodeFind == null)
					{
						nodeFind = service.clickIfFindOne(recyclerNode.getChild(index), "互相关注", new NodeHelper.OnFindCallback()
						{
							@Override
							public boolean find(AccessibilityNodeInfo theNode, int indent, String className, String text, String contentDescription, String viewId)
							{
								return !TextUtils.isEmpty(className) && className.equals("android.widget.TextView") && //
										!TextUtils.isEmpty(text) && text.equals("互相关注");
							}
						}, null);
					}
					if (nodeFind != null)
