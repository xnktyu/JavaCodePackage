...
public class OperateDyYingLiu extends OperateBase
{
	...
	public OperateDyYingLiu(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runSpeed = ActivityDyYingLiu.readRunSpeed(context);
	...
	{
		if (Math.random() * 100 < ActivityDyYingLiu.readRunCount(context))
			clickHeart();
		else
			whetherClickPingLun();
	}
	...
	{
		if (!TextUtils.isEmpty(ActivityDyYingLiu.randomReadMsg(context)))
			clickPingLun();
		else
			swipeNext();
	}
	...
	private void inputText()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String atTarget = ActivityDyYingLiu.readRunTarget(context);
				if (TextUtils.isEmpty(atTarget))
				{
					...
					}, new DyAccessibilityService.OnClickFunction()
					{
						...
						{
							String msg = ActivityDyYingLiu.randomReadMsg(context);
							LOG.v("输入 " + msg);
