...
public class OperateDummy015 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy015(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy015.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy015.readRunSpeed(context);
		runTarget = ActivityDummy015.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy015.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy015.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy015.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy015.addRunRecord(context);
						startOne();
					}
				}
			}
