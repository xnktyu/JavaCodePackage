...
public class OperateDyFenGuan extends OperateBase
{
	...
	public OperateDyFenGuan(DyAccessibilityService service)
	{
	...
	public void start()
	{
		allCount = ActivityDyFenGuan.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDyFenGuan.readRunSpeed(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				service.clickIfFindOne("RecyclerView", new NodeHelper.OnFindCallback()
				{
					...
					{
						return !TextUtils.isEmpty(className) && (className.equals("android.support.v7.widget.RecyclerView") || className.equals("androidx.recyclerview.widget.RecyclerView")) && theNode.isScrollable() && indent == 2;
					}
					...
					AccessibilityNodeInfo nodeFind = service.clickIfFindOne(recyclerNode.getChild(index), "关注", new NodeHelper.OnFindCallback()
					{
						...
						public boolean find(AccessibilityNodeInfo theNode, int indent, String className, String text, String contentDescription, String viewId)
						{
							return !TextUtils.isEmpty(className) && className.equals("android.widget.TextView") && //
									!TextUtils.isEmpty(text) && (text.equals("关注") || text.equals("回关"));
						}
					}, null);
					if (nodeFind != null)
					{
