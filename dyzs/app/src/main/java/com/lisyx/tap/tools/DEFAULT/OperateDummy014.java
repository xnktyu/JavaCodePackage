...
public class OperateDummy014 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy014(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy014.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy014.readRunSpeed(context);
		runTarget = ActivityDummy014.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy014.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy014.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy014.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy014.addRunRecord(context);
						startOne();
					}
				}
			}
