...
public class OperateDummy013 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy013(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy013.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy013.readRunSpeed(context);
		runTarget = ActivityDummy013.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy013.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy013.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy013.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy013.addRunRecord(context);
						startOne();
					}
				}
			}
