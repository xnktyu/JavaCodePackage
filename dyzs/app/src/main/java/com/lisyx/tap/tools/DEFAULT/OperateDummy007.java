...
public class OperateDummy007 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy007(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy007.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy007.readRunSpeed(context);
		runTarget = ActivityDummy007.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy007.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy007.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy007.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy007.addRunRecord(context);
						startOne();
					}
				}
			}
