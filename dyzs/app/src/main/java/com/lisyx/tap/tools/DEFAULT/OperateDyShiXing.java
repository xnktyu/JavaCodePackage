...
public class OperateDyShiXing extends OperateBase
{
	...
	public OperateDyShiXing(DyAccessibilityService service)
	{
	...
	public void start()
	{
		allCount = ActivityDyShiXing.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDyShiXing.readRunSpeed(context);
		LOG.v(String.format("allCount = %s, runSpeed = %s", allCount, runSpeed));
		if (!TextUtils.isEmpty(ActivityDyShiXing.randomReadMsg(context)))
			startOne();
		else
		...
		if (index < recyclerNode.getChildCount())
		{
			service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
			{
				...
				public void process()
				{
					...
					recyclerNode.getChild(index).performAction(AccessibilityNodeInfo.ACTION_CLICK);
	...
	private void checkMsg(final AccessibilityNodeInfo recyclerNode, final int index)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDyShiXing.randomReadMsg(context);
