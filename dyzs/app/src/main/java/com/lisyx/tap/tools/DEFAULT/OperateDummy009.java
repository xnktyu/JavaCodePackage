...
public class OperateDummy009 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy009(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy009.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy009.readRunSpeed(context);
		runTarget = ActivityDummy009.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy009.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy009.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy009.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy009.addRunRecord(context);
						startOne();
					}
				}
			}
