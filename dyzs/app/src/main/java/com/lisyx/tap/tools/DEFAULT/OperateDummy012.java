...
public class OperateDummy012 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy012(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy012.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy012.readRunSpeed(context);
		runTarget = ActivityDummy012.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy012.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy012.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy012.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy012.addRunRecord(context);
						startOne();
					}
				}
			}
