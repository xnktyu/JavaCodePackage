...
public class OperateDummy008 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy008(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy008.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy008.readRunSpeed(context);
		runTarget = ActivityDummy008.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy008.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy008.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy008.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy008.addRunRecord(context);
						startOne();
					}
				}
			}
