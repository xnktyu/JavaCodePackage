...
public class OperateDummy011 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy011(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy011.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy011.readRunSpeed(context);
		runTarget = ActivityDummy011.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy011.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy011.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy011.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy011.addRunRecord(context);
						startOne();
					}
				}
			}
