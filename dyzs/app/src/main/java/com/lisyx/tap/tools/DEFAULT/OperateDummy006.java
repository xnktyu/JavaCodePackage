...
public class OperateDummy006 extends OperateBase
{
	private DyAccessibilityService service = null;

	public OperateDummy006(DyAccessibilityService service)
	{
	...
	public void start()
	{
		runCount = ActivityDummy006.readRunCount(context);
		currCount = 0;
		runSpeed = ActivityDummy006.readRunSpeed(context);
		runTarget = ActivityDummy006.readRunTarget(context);
		...
		if (TextUtils.isEmpty(ActivityDummy006.randomReadMsg(context)))
		{
	...
	private void gotoChenYuanList()
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				if (nodeFind != null)
				{
					findChenYuanList(-1, ActivityDummy006.readRunRecord(context), 0);
				}
			}
		});
	}
	...
	private void checkMsg(final int targetIndex)
	{
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				final String msg = ActivityDummy006.randomReadMsg(context);
		...
		service.wait(runSpeed, new DyAccessibilityService.OnWaitCallback()
		{
			...
			public void process()
			{
				...
				else
				{
					...
					{
						ActivityDummy006.addRunRecord(context);
						startOne();
					}
				}
			}
